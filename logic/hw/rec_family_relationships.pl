...
likes_all_magical_creatures(harry):- not(dementor).
...
ancestor_of(X,Y):- grandfather_of(X,Y); grandmother_of(X,Y).
ancestor_of(X,Y):- grandmother_of(X,Z), ancestor_of(Z,Y).
ancestor_of(X,Y):- grandfather_of(X,Z), ancestor_of(Z,Y).
not_parent(X,Y):- not(father_of(X,Y)), not(mother_of(X,Y)).
